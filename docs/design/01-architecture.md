...
5. **文字起こしは記録を作らない。**完了したジョブはraw_textとsegmentsを持つだけで、ENTRYには触れない。結果はSSEで、いま開いている新規作成または編集のフォームの本文欄の末尾に流れ込む（既に本文があれば空行を1つ挟む）。利用者が手直しして「記録する」「保存」を押すと、`POST /api/entries` または `PATCH /api/entries/:id` に `transcription_job_ids`（文字列の配列）が添えて送られ、サーバーはそこで初めて各ジョブと、そのジョブが持つ音声の添付を、その記録のものにする（`server/services/transcribe.ts` の `attachJobsToEntry`）。1つの記録に何度でも録音を足せる。存在しないジョブのidと、既に別の記録に結びついたジョブは400。
   - こうしている理由：音声から起こした文はほぼ必ず手直ししたい。完成したカードが勝手に並ぶと、直すために「編集」を開き直すことになる。書いている場所にそのまま流し込めば、そのまま直して保存できる。
   - 保存されないまま残ったジョブ（entry_idがNULLのまま）は、音声ファイルごと残る。掃除は未実装（個人用のため当面許容。設計02の残課題）。
   - 書きかけのフォームは、入力と文字起こしの反映のたびに`localStorage`へ下書きとして写す（キーは`draft:new:<学習日>`と`draft:edit:<記録のid>`。中身は見出し・本文・ジョブのid）。保存前に再読み込みしても戻る。「記録する」「保存」「やめる」で消す。
